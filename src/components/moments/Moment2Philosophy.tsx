...
function buildWordSpans(text: string) {
  return text.split(' ').map((word, i) => (
    <span
      key={i}
      className="q-word"
      style={{ display: 'inline-block', opacity: 0, transform: 'translateY(40px) scale(0.97)', marginRight: '0.22em', willChange: 'transform, opacity' }}
    >
      {word}
    </span>
  ));
...
export default function Moment2Philosophy() {
  ...
  useEffect(() => {
    ...
    // Q1 entrance when section comes into view
    ...
    // Scrubbed transitions — scrub:1.2 adds smoothing so fast scroll doesn't skip frames
  ...
  return (
    <section id="moment-philosophy" ref={outerRef} style={{ height: '180vh', position: 'relative' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        {QUOTES.map((q, idx) => (
          <div
            key={idx}
            ref={refs[idx]}
            style={{ position: 'absolute', textAlign: 'center', maxWidth: 900, padding: '0 40px', opacity: 0, willChange: 'opacity, transform' }}
          >
            ...
            <div
              className="q-sub"
              style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '1.1rem', letterSpacing: '0.1em', color: '#C4A35A', opacity: 0, transform: 'translateY(16px)' }}
            >
              {q.sub}
            </div>
          </div>
